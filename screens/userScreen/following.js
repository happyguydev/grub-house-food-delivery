...
export default class following extends Component {
  ...
  constructor(props) {
    super(props);
    console.warn(props.navigation.state.params.reload, "reload")
    this.state = {
  ...
  handleSearch=(v)=>{
    ...
    this.state.following&&this.state.following.map((man, index)=>{
      var first_name = man.first_name?man.first_name.toLowerCase():""
      var last_name = man.last_name?man.last_name.toLowerCase():""
  ...
  setDataFromPP = (data)=>{
    console.log("set dta from pp")
    this.setState({
      following: data.following,
      followers: data.followers,
      filteredFollowingData: data.following,
      filteredFollowersData:data.followers
    })
        ...
        {this.state.selectedTab==1&&<FlatList
          ...
          <View style={styles.flatlistContainer}>
            <TouchableOpacity 
              ...
              onPress= {()=>{
                console.log(this.props.navigation.state.params.reload, "reload")
                console.log(d)
                var client_info = {
                  avatar:d.avatar,
                  bgImage: d.bgImage,
                  ...
                  follower_id: d.follwer_id,
                  id: d.id,
                  last_name: d.last_name,
                  notification_flag: d.notification_flag, 
                }
                this.props.navigation.navigate("PublicProfile", {
                  client: client_info,
                  reload: this.props.navigation.state.params.reload,
                  setData: this.setDataFromPP
                })
              }}
            >
            {d.avatar?<Avatar rounded size={60} source={{ uri: "https://www.grubhouse.co.uk/upload/"+d.avatar }} />:<Avatar rounded size={60} title={d?.first_name?d?.first_name[0]:null+d?.last_name?d?.last_name[0]:null} />}
        ...
        {this.state.selectedTab==0&&<FlatList
          ...
          renderItem={({ item: d }) => (
            <View style={styles.flatlistContainer}>
              ...
              >
                {d.avatar?<Avatar rounded size={60} source={{ uri: "https://www.grubhouse.co.uk/upload/"+d.avatar }} />:<Avatar rounded size={60} title={d?.first_name?d?.first_name[0]:"N"} />}
                <Text style={styles.nameText}>{d?.first_name}&nbsp;&nbsp;{d?.last_name}</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.removeContainer1}
                onPress = {()=>{this.toggleFollow(d.follower_id, d, "remove")}}
              >
        ...
        {this.state.selectedTab==2&&<FlatList
          ...
          renderItem={({ item: d }) => (
            <View style={styles.flatlistContainer}>
              ...
              {d.avatar?<Avatar
                          rounded size={60} 
                          source={{ uri: "https://www.grubhouse.co.uk/upload/"+d.avatar }} 
                          />:<Avatar 
                              rounded
                              size={60}
                              title={d?.first_name[0]+d?.last_name[0]}
                              />}
                <Text style={styles.nameText} numberOfLines={1}>
                  {d?.first_name}&nbsp;&nbsp;{d?.last_name}
                </Text>
